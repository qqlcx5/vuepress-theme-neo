...
export const setupPageData = () => {
    const _filterPosts = computed(() => filterPosts(pagesData))
    const _sortPosts = computed(() => sortPosts(_filterPosts.value))
    const _sortPostsByDate = computed(() => sortPostsByDate(_filterPosts.value))
    const _groupPosts = computed(() => groupPosts(_sortPostsByDate.value))
    const _categoriesAndTags = computed(() => categoriesAndTags(_groupPosts.value))
